  ...
  return (
    <div className="border border-[#7fee64]/20 bg-[#0d180a] p-3 text-[12px] font-mono leading-[1.35] rounded">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[#7fee64]/80">
        ...
        <div><span className="text-white/60">Implication:</span> <span className="text-[#d8f9d9]">Tile density {tilesA}→{tilesB} (+{deltaTiles}) faithful no 64 cap, HBM {a.hbm.totalGB}GB→{b.hbm.totalGB}GB (+{hbmDelta}GB) {b.hbm.version} {b.dualDie?'dual-die CoWoS-L interposer reticle-limit workaround':''}</span></div>
      </div>
      <div className="mt-2 text-[12px] text-[#7fee64]/70 leading-[1.3]">
        <span className="text-white/60">Tension:</span> {from==='h100-sxm5' && to==='b200-sxm' ? 'Single reticle 80B transistor wall hits yield – dual-die interposer workaround enables larger die but complicates NVLink topology; 1/3 brain is visual 10-lane superhighway – seeing scale helps Intelligence Lift teaching.' : ''}
